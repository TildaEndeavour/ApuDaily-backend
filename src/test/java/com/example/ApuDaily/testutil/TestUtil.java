...
public class TestUtil implements Util {
  ...
  public Tag createTag(int seed) {
    return Tag.builder().id((long) seed).name("Tag " + seed).build();
  }

  @Override
  public Role createUserRole() {
    return Role.builder().id((long) 1).name("ROLE_USER").build();
  }

  @Override
  public Role createAdminRole() {
    return Role.builder().id((long) 2).name("ROLE_ADMIN").build();
  }
